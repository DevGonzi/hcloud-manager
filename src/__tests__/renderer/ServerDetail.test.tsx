...
const mockServer: HCloudServer = {
  ...
  public_net: { ipv4: { ip: '1.2.3.4', blocked: false }, ipv6: { ip: '2a01::1/64', blocked: false } },
...
describe('ServerDetail', () => {
  it('panel ist geschlossen wenn kein Server ausgewählt', () => {
    vi.mocked(useServerStore).mockReturnValue({ servers: [], selectedServerId: null, selectServer: vi.fn() } as any)
    const { container } = render(<ServerDetail projectId="p1" readonly={false} onAction={vi.fn()} />)
    expect(container.firstChild).toHaveClass('translate-x-full')
  })

  it('zeigt Server-Namen wenn ausgewählt', () => {
    vi.mocked(useServerStore).mockReturnValue({ servers: [mockServer], selectedServerId: 42, selectServer: vi.fn() } as any)
